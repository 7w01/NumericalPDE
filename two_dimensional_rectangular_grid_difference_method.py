...
for ni in range(1, NK+1):
    ...
    Gamma4 = a4 + np.linspace(0,N,N+1)*(M + 1)
    ...
    for i in range(1, len(Gamma2) - 1):
        ...
        b[Gamma2[i]-1] = 0.5 * 2 * np.pi ** 2 * tempx * tempy \
    ...
    for i in range(1, len(Gamma3) - 1):
        ...
        b[Gamma3[i]-1] = 0.5 * 2 * np.pi ** 2 * tempx * tempy \
    ...
    b[(N + 1) * (M + 1)-1] = 0.25 * 2 * np.pi ** 2 * tempx * tempy\
    - ((0.5 * tempx ** 2 + np.cos(np.pi * tempx)) - (0.5 * tempx2 ** 2 + np.cos(np.pi * tempx2))) / hx / hy\
    -((0.5 * tempy ** 2 + np.cos(np.pi * tempy)) - (0.5 * tempy2 ** 2 + np.cos(np.pi * tempy2))) / hx / hy
    ...
    print(1)
    errC[ni-1] = np.max(np.abs(utrue - u))

    errL[ni-1] = (utrue - u).dot((utrue-u)*hx*hy)
    errL[ni-1] = np.sqrt(errL[ni-1])

    meshsize[ni-1] = np.sqrt(hx ** 2 + hy ** 2)
